...
impl CloudAICompanion {
    ...
    async fn resolve_project_via_extension_flow(
        ...
        if let Some(project) = extract_project_id(
            load.get("cloudaicompanionProject")
                .or_else(|| load.get("response").and_then(|v| v.get("cloudaicompanionProject"))),
        ) {
    ...
    async fn request_setup_get(&self, token: &str, operation_name: &str) -> anyhow::Result<JsonValue> {
