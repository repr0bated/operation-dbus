...
impl ProjectionEngine {
    ...
    pub async fn discover_all(&self, registry: &crate::registry::ToolRegistry, bus_type: BusType) -> Result<usize> {
        ...
        tracing::info!("Discovering tools for {} services on {:?} bus", services.len(), bus_type);

        // Process each service
        for service in services {
            tracing::debug!("Introspecting service '{}' on {:?} bus...", service, bus_type);
                    ...
                    for iface in info.interfaces {
                        ...
                        for method in iface.methods {
                            ...
                            let definition = crate::registry::ToolDefinition {
                                ...
                                description: format!("D-Bus method {}.{} on {} at {}", iface.name, method.name, service, path),
                                ...
                                tags: vec!["dbus".to_string(), "projected".to_string(), service.clone()],
                            };

                            if let Ok(_) = registry.register(tool.name.clone().into(), Arc::new(tool), definition).await {
                                service_tools += 1;
                            }
                        }
                    }
            ...
            if service_tools > 0 {
                tracing::info!("  → Service {}: registered {} tools from {} paths", service, service_tools, paths.len());
            }
        }
    ...
    fn discover_paths<'a>(&'a self, bus_type: BusType, service: &'a str, path: &'a str, depth: usize) -> Pin<Box<dyn Future<Output = Vec<String>> + Send + 'a>> {
        Box::pin(async move {
        ...
        if let Ok(info) = self.introspection.introspect(bus_type, service, path).await {
            for child in &info.children {
                ...
                let child_paths = self.discover_paths(bus_type, service, &child_path, depth + 1).await;
