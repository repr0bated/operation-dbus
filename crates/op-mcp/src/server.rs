...
impl McpServer {
    ...
    async fn handle_tools_call(&self, request: McpRequest) -> McpResponse {
        ...
        let tool_name = match params.as_object().and_then(|obj| obj.get("name")).and_then(|n| n.as_str()) {
            Some(n) => n,
            None => return McpResponse::error(
                request.id,
        ...
        let arguments = params.as_object().and_then(|obj| obj.get("arguments")).cloned().unwrap_or(json!({}));
    ...
    async fn handle_compact_tool(&self, request: McpRequest) -> McpResponse {
        ...
        match request.method.as_str() {
            "list_tools" => {
                let category = params.as_object().and_then(|obj| obj.get("category")).and_then(|c| c.as_str());
                let limit = params.as_object().and_then(|obj| obj.get("limit")).and_then(|l| l.as_u64()).unwrap_or(20) as usize;
            ...
            "search_tools" => {
                let query = params.as_object().and_then(|obj| obj.get("query")).and_then(|q| q.as_str()).unwrap_or("");
                let limit = params.as_object().and_then(|obj| obj.get("limit")).and_then(|l| l.as_u64()).unwrap_or(10) as usize;
            ...
            "get_tool_schema" => {
                let tool_name = params.as_object().and_then(|obj| obj.get("tool_name")).and_then(|n| n.as_str()).unwrap_or("");
