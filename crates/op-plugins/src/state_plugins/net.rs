...
impl StatePlugin for NetStatePlugin {
    ...
    async fn calculate_diff(&self, current: &Value, desired: &Value) -> Result<StateDiff> {
        ...
        for (name, desired_iface) in &desired_map {
            if let Some(current_iface) = current_map.get(name) {
                ...
                if simd_json::serde::to_owned_value(current_iface)? != simd_json::serde::to_owned_value(desired_iface)? {
        ...
        for action in &diff.actions {
            match action {
                ...
                | StateAction::Modify {
                    ...
                    let iface_config: InterfaceConfig = simd_json::serde::from_owned_value(config.clone())?;
    ...
    async fn rollback(&self, checkpoint: &Checkpoint) -> Result<()> {
        let old_config: NetworkConfig = simd_json::serde::from_owned_value(checkpoint.state_snapshot.clone())?;
