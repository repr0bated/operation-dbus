...
impl McpStatePlugin {
    ...
    async fn apply_server_config(
        ...
        job.result = Some(op_state_store::ExecutionResult {
            success: true,
            output: Some(simd_json::serde::to_owned_value("Server configured successfully")?),
            error: None,
        });
    ...
    async fn apply_tool_groups_config(&self, config: &ToolGroupsConfig) -> Result<()> {
        ...
        job.result = Some(op_state_store::ExecutionResult {
            success: true,
            output: Some(simd_json::serde::to_owned_value("Tool groups configured successfully")?),
            error: None,
        });
        self.state_store.update_job(&job).await?;
...
impl StatePlugin for McpStatePlugin {
        ...
        for action in &diff.actions {
            if let StateAction::Modify { resource, changes } = action {
                let result = if resource.starts_with("server:") {
                    ...
                    let server_config: McpServerConfig = simd_json::serde::from_owned_value(changes.clone())?;
    ...
    async fn rollback(&self, checkpoint: &Checkpoint) -> Result<()> {
        let old_config: McpConfig = simd_json::serde::from_owned_value(checkpoint.state_snapshot.clone())?;
