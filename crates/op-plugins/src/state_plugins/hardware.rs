...
impl HardwarePlugin {
    ...
    async fn get_cpu_info() -> CpuInfo {
        ...
        if cores == 0 {
             cores = content.lines().filter(|l| l.starts_with("processor")).count();
        }
                 ...
                 if let Ok(mut val) = simd_json::to_owned_value(&mut json_str.as_bytes().to_vec()) {
                     if let Some(blockdevices) = val.get("blockdevices").and_then(|v| v.as_array()) {
                         for dev in blockdevices {
                             let name = dev.get("name").and_then(|s| s.as_str()).unwrap_or("unknown").to_string();
                             let size = dev.get("bytes").and_then(|s| s.as_str().or(Some("0"))).and_then(|s| s.parse::<u64>().ok()).unwrap_or(0);
                             let mountpoint = dev.get("mountpoint").and_then(|s| s.as_str()).map(|s| s.to_string());
