             ...
             if output.status.success() {
                 if let Ok(json) = std::str::from_utf8(&output.stdout) {
                     if let Ok(val) = simd_json::to_owned_value(&mut json.as_bytes().to_vec()) {
                         account = val.get("core").and_then(|c| c.get("account")).and_then(|v| v.as_str()).map(|s| s.to_string());
                         project_id = val.get("core").and_then(|c| c.get("project")).and_then(|v| v.as_str()).map(|s| s.to_string());
                     }
                 }
             }
        }
