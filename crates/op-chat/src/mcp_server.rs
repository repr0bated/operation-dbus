...
use op_mcp::grpc::proto::{
    CallToolRequest, CallToolResponse, GetToolSchemaRequest, GetToolSchemaResponse,
    HealthRequest, HealthResponse, InitializeRequest, InitializeResponse, ListToolsRequest,
    ListToolsResponse, McpError as ProtoMcpError, McpEvent as ProtoMcpEvent,
    McpRequest as ProtoMcpRequest, McpResponse as ProtoMcpResponse, SubscribeRequest,
    ToolOutput as ProtoToolOutput,
};
...
fn prost_value_to_json(kind: &prost_types::value::Kind) -> Value {
    match kind {
        ...
        prost_types::value::Kind::BoolValue(b) => {
            Value::Static(simd_json::StaticNode::Bool(*b))
        }
        prost_types::value::Kind::StructValue(s) => {
            struct_to_value(s).unwrap_or_else(|| json!({}))
        }
...
fn json_to_prost_value(v: &Value) -> prost_types::Value {
    let kind = match v {
        ...
        Value::Static(simd_json::StaticNode::F64(n)) => {
            prost_types::value::Kind::NumberValue(*n)
        }
...
impl McpService for ChatMcpServer {
    ...
    async fn call_tool(
        ...
        {
            Ok(tracked) => {
                let result_struct = tracked
                    .result
                    .result
                    .as_ref()
                    .and_then(value_to_struct);
    ...
    async fn get_tool_schema(
        ...
        let req = request.into_inner();
        match self.chat_actor.tool_registry().get_definition(&req.tool_name).await {
