...
impl Aggregator {
    ...
    async fn compact_list_tools(&self, args: Value) -> Result<Value> {
        let category = args.as_object().and_then(|obj| obj.get("category")).and_then(|v| v.as_str());
        let namespace = args.as_object().and_then(|obj| obj.get("namespace")).and_then(|v| v.as_str());
        let limit = args.as_object().and_then(|obj| obj.get("limit")).and_then(|v| v.as_u64()).unwrap_or(20) as usize;
