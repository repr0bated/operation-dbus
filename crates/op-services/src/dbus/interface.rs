...
impl DbusInterface {
    ...
    async fn get_status(&self, name: &str) -> zbus::fdo::Result<String> {
        ...
        let status = self.manager.get_status(&name).await
            .map_err(|e| zbus::fdo::Error::Failed(e.to_string()))?;
    ...
    #[zbus(signal)]
    async fn service_state_changed(ctx: &SignalContext<'_>, name: &str, old_state: &str, new_state: &str) -> zbus::Result<()>;
}
...
pub async fn run_dbus_server(manager: Arc<ServiceManager>) -> anyhow::Result<()> {
    ...
    conn.object_server().at("/org/opdbus/services", iface).await?;
