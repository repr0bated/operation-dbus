...
impl SyncEngine {
    ...
    async fn dbus_connection(&self) -> Result<Connection, SyncError> {
    ...
    async fn apply_mutation_via_state_manager(
        ...
        let request_json = simd_json::to_string(&request)
            .map_err(|e| SyncError::Validation(format!("canonical mutation encode error: {}", e)))?;
    ...
    async fn apply_mutation_via_plugin_surface(
        ...
        match change_type {
            ChangeType::PropertySet => {
                ...
                let zbus_value = simd_json_to_zvariant(&prop_value)
                    .map_err(|e| SyncError::Validation(format!("Property conversion error: {}", e)))?;
