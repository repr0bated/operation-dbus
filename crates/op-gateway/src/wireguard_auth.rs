...
use simd_json::{OwnedValue, owned::Object as SimdObject};
use sqlx::Row;

use ring::{digest, hkdf, rand::SystemRandom};
use x25519_dalek::{PublicKey, SharedSecret};
use chacha20poly1305::{ChaCha20Poly1305, Key, AeadInPlace};
...
impl WireGuardAuthManager {
    ...
    pub async fn create_session(&self, peer_pubkey: &str, client_info: Option<ClientInfo>) -> Result<WireGuardSession> {
        debug!("Creating WireGuard session for peer: {}", peer_pubkey);
        
        // Validate peer public key format
        if !Self::is_valid_pubkey(peer_pubkey) {
            return Err(anyhow::anyhow!("Invalid peer public key format: {}", peer_pubkey));
        }
            ...
            if let Some(existing_session_id) = peer_sessions.get(peer_pubkey) {
                ...
                if let Some(session) = sessions.get(existing_session_id) {
                    if session.is_active && session.expires_at > Self::current_timestamp() {
                        debug!("Returning existing active session for peer: {}", peer_pubkey);
    ...
    pub async fn list_sessions(&self, filter: Option<SessionFilter>) -> Result<Vec<WireGuardSession>> {
        let sessions = self.sessions.read().await;
    ...
    pub async fn rotate_session_key(&self, peer_pubkey: &str, force: bool) -> Result<String> {
        ...
        // Update session
        {
            ...
            if let Some(session) = sessions.get_mut(&session_id) {
                ...
                session.flags.insert("current_session_key".to_string(), new_session_key.clone());
    ...
    async fn derive_psk(&self, peer_pubkey: &str) -> anyhow::Result<String> {
        ...
        let peer_key_bytes = match hex::decode(peer_pubkey) {
            Ok(bytes) => {
                if bytes.len() == 32 {
                    ...
                    return Err(anyhow::anyhow!("Invalid peer public key length: {}", bytes.len()));
                }
            }
            Err(_) => return Err(anyhow::anyhow!("Invalid peer public key format: {}", peer_pubkey)),
        };
        ...
        if let Some(psk) = psks.first() {
            ...
            if let Err(e) = key_storage.store_key(&psk_key_id, psk, KeyType::WireGuardPsk).await {
    ...
    async fn derive_session_key(&self, peer_pubkey: &str) -> anyhow::Result<String> {
        ...
        let session_keys = self.crypto_engine.derive_session_keys(&[peer_key_bytes], &[server_nonce]);
    ...
    pub async fn store_private_key(&self, key_id: &str, private_key: &[u8; 32]) -> Result<()> {
        let mut key_storage = self.key_storage.lock().await;
        key_storage.store_key(key_id, private_key, KeyType::WireGuardPrivate).await?;
    ...
    pub async fn retrieve_private_key(&self, key_id: &str) -> anyhow::Result<[u8; 32]> {
        ...
        if key_data.len() != 32 {
            return Err(anyhow::anyhow!("Invalid private key length: {}", key_data.len()));
        }
    ...
    pub async fn get_storage_stats(&self) -> anyhow::Result<crate::encrypted_storage::StorageStats> {
        let key_storage = self.key_storage.lock().await;
    ...
    async fn load_sessions_from_database(&self) -> anyhow::Result<()> {
        ...
        info!("Loaded {} active WireGuard sessions from database", active_count);
        Ok(())
    }
            ...
            loop {
                ...
                // Find expired sessions
                {
                    ...
                    for (session_id, session) in sessions_read.iter() {
                        if !session.is_active || session.expires_at <= now {
                            expired_sessions.push((session_id.clone(), session.peer_pubkey.clone()));
                        }
                    }
                }
                ...
                if !expired_sessions.is_empty() {
                    ...
                    let active_sessions = sessions_write.values()
                        .filter(|s| s.is_active && s.expires_at > now)
                        .count() as u64;
...
impl SimdCryptoEngine {
    ...
    pub fn derive_session_keys(
        ...
        for (peer_key, server_nonce) in peer_keys.iter().zip(server_nonces) {
            ...
            if argon2.hash_password_into(&input, salt, &mut session_key).is_ok() {
