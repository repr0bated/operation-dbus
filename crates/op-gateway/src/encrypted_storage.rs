...
impl EncryptedKeyStorage {
    ...
    pub async fn new(config: EncryptedStorageConfig) -> Result<Self> {
        info!("Initializing encrypted key storage at {:?}", config.base_path);
    ...
    async fn setup_native_btrfs_encryption(&self) -> Result<()> {
        info!("Setting up native Btrfs encryption for subvolume: {}", self.config.subvolume_name);
        
        // Check if subvolume already exists
        if self.storage_path.exists() {
            debug!("Encrypted subvolume already exists: {:?}", self.storage_path);
            return Ok(());
        }
        ...
        if !output.status.success() {
            ...
            if stderr.contains("encryption not supported") || stderr.contains("invalid option") {
                ...
                return Err(anyhow::anyhow!(
                    "Failed to create encrypted subvolume: {}", stderr
                ));
            }
        } else {
    ...
    async fn setup_luks_encryption(&self) -> Result<()> {
        let device_name = self.config.luks_device_name.as_ref()
    ...
    /// Store encrypted key
    pub async fn store_key(&mut self, key_id: &str, key_data: &[u8], key_type: KeyType) -> anyhow::Result<()> {
        if !self.is_initialized {
            return Err(anyhow::anyhow!("Storage not initialized"));
        }
        
        let master_key = self.master_key.as_mut()
        ...
        cipher.encrypt_in_place(&Nonce::from_slice(&nonce), b"", &mut encrypted_data)
    ...
    pub async fn retrieve_key(&self, key_id: &str) -> anyhow::Result<Vec<u8>> {
        ...
        cipher.decrypt_in_place(&Nonce::from_slice(&entry.nonce), b"", &mut decrypted_data)
    ...
    pub async fn get_stats(&self) -> anyhow::Result<StorageStats> {
        ...
        Ok(StorageStats {
            ...
            is_encrypted: self.config.use_native_encryption || self.config.luks_device_name.is_some(),
