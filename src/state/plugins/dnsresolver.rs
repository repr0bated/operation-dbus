...
impl DnsResolverPlugin {
    ...
    fn parse_resolv_conf(text: &str) -> DnsItem {
        ...
        for line in text.lines() {
            ...
            if let Some(keyword) = parts.next() {
                match keyword {
                    "nameserver" => {
                        if let Some(ip) = parts.next() { servers.push(ip.to_string()); }
                    }
                    "search" => {
                        let vals: Vec<String> = parts.map(|v| v.to_string()).collect();
                        if !vals.is_empty() { search = Some(vals); }
                    }
                    "options" => {
                        let vals: Vec<String> = parts.map(|v| v.to_string()).collect();
                        if !vals.is_empty() { options = Some(vals); }
                    }
                    _ => {}
                }
            }
        }
        DnsItem { id: "resolvconf".into(), mode: Mode::ObserveOnly, servers, search, options }
    }

    fn read_resolv_conf() -> String {
        if let Ok(out) = Command::new("cat").arg("/etc/resolv.conf").output() {
            if out.status.success() { return String::from_utf8(out.stdout).unwrap_or_default(); }
        }
    ...
    fn write_resolv_conf(item: &DnsItem) -> Result<()> {
        ...
        if let Some(sr) = &item.search {
            if !sr.is_empty() {
                buf.push_str("search "); buf.push_str(&sr.join(" ")); buf.push('\n');
            }
        }
        if let Some(opts) = &item.options {
            if !opts.is_empty() {
                buf.push_str("options "); buf.push_str(&opts.join(" ")); buf.push('\n');
            }
        }
        for ns in &item.servers { buf.push_str("nameserver "); buf.push_str(ns); buf.push('\n'); }
    ...
    fn equal_desired(cur: &DnsItem, want: &DnsItem) -> bool {
        Self::normalize(&cur.servers) == Self::normalize(&want.servers)
            && cur.search.as_ref().map(|v| Self::normalize(v)) == want.search.as_ref().map(|v| Self::normalize(v))
            && cur.options.as_ref().map(|v| Self::normalize(v)) == want.options.as_ref().map(|v| Self::normalize(v))
    }
...
impl StatePlugin for DnsResolverPlugin {
    ...
    async fn calculate_diff(&self, _current: &Value, desired: &Value) -> Result<StateDiff> {
        let want: DnsState = match serde_json::from_value(desired.clone()) {
            Ok(v) => v, Err(_) => DnsState { version: 1, items: Vec::new() },
        };
        ...
        for item in &want.items {
            match cur {
                Some(c) if Self::equal_desired(c, item) => actions.push(StateAction::NoOp { resource: item.id.clone() }),
                Some(_) => actions.push(StateAction::Modify { resource: item.id.clone(), changes: serde_json::to_value(item).unwrap_or(serde_json::json!({})) }),
                None => actions.push(StateAction::Create { resource: item.id.clone(), config: serde_json::to_value(item).unwrap_or(serde_json::json!({})) }),
            }
        }
        let meta = DiffMetadata {
            timestamp: chrono::Utc::now().timestamp(),
            current_hash: format!("{:x}", md5::compute(serde_json::to_string(&serde_json::json!({"items": cur_all})).unwrap_or_default())),
            desired_hash: format!("{:x}", md5::compute(serde_json::to_string(&want).unwrap_or_default())),
        };
        Ok(StateDiff { plugin: self.name().to_string(), actions, metadata: meta })
    }
    ...
    async fn apply_state(&self, diff: &StateDiff) -> Result<ApplyResult> {
        ...
        for action in &diff.actions {
            match action {
                ...
                StateAction::Modify { resource, changes: config } => {
                    let item: DnsItem = match serde_json::from_value(config.clone()) {
                        Ok(v) => v, Err(_) => { errors.push(format!("{}: invalid payload", resource)); continue; }
                    };
                    match item.mode {
                        Mode::ObserveOnly => changes_applied.push(format!("{}: no action required", resource)),
                        Mode::Enforce => match Self::write_resolv_conf(&item) {
                            Ok(_) => changes_applied.push(format!("{}: resolv.conf updated", resource)),
    ...
    async fn verify_state(&self, desired: &Value) -> Result<bool> {
        let want: DnsState = match serde_json::from_value(desired.clone()) { Ok(v) => v, Err(_) => return Ok(true) };
        let cur_all = Self::query_system();
        let cur = match cur_all.get(0) { Some(v) => v, None => return Ok(false) };
        for item in &want.items {
            if !Self::equal_desired(cur, item) { return Ok(false); }
        }
        Ok(true)
    }
