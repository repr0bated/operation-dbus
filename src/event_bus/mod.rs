...
type EventArc = Arc<EventBox>;
type EventHandlerFn = dyn Fn(EventArc) -> Result<()> + Send + Sync;
...
impl FnHandler {
    ...
    where
        F: Fn(EventArc) -> Result<()> + Send + Sync + 'static,
    {
        Self {
...
type HandlerVec = Vec<Arc<Box<dyn EventHandler>>>;
type HandlerMap = HashMap<String, HandlerVec>;
type ChannelMap = HashMap<String, broadcast::Sender<EventArc>>;
...
pub struct EventBus {
    ...
    history: Arc<RwLock<Vec<EventArc>>>,
...
pub trait EventInterceptor: Send + Sync {
    ...
    async fn before_publish(&self, event: &dyn Event) -> Result<()>;

    /// Called after event is published
    async fn after_publish(&self, event: &dyn Event);
}
...
impl EventBus {
    ...
    pub async fn stream(&self, event_type: &str) -> broadcast::Receiver<EventArc> {
        let mut channels = self.channels.write().await;
        let sender = channels.entry(event_type.to_string()).or_insert_with(|| {
    ...
    pub async fn publish(&self, event: Box<dyn Event>) -> Result<()> {
        let event_arc: EventArc = Arc::new(event);
...
impl EventInterceptor for MetricsInterceptor {
    async fn before_publish(&self, _event: &dyn Event) -> Result<()> {
        Ok(())
    }
